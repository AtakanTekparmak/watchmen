...
def stage_workspace(task: Dict[str, Any], workspace: Path) -> Dict[str, Any]:
    ...
    if kind == "skillsbench_test_sh":
        return _stage_skillsbench_workspace(payload, workspace)
...
def _stage_tblite_workspace(
    ...
    try:
        ...
        if proc.returncode != 0:
            logger.warning("docker create failed for %s: %s", image, proc.stderr[-300:])
            return {
                "docker_image": image,
                "container_app_dir": "/app",
                "staging_skipped": True,
        ...
        if cp.returncode != 0:
            logger.warning("docker cp /app failed for %s: %s", image, cp.stderr[-300:])
            return {
                "docker_image": image,
                "container_app_dir": "/app",
                "staging_skipped": True,
...
def _stage_skillsbench_workspace(
    payload: Dict[str, Any],
    workspace: Path,
) -> Dict[str, Any]:
    """Copy the SkillsBench task's ``environment/`` into ``workspace``.

    Mirrors the upstream layout — the verifier (``test.sh``) expects
    the env's pre-baked files in the workdir. If ``workspace`` already
    has content (e.g. the runner staged something earlier), we leave
    it alone and only ensure ``environment/`` exists.
    """
    env_src = payload.get("environment_dir")
    if not env_src:
        logger.warning("skillsbench task missing environment_dir; skipping stage")
        return {}
    env_src_path = Path(env_src)
    if not env_src_path.exists():
        logger.warning("skillsbench environment_dir does not exist: %s", env_src)
        return {"environment_dir": env_src, "staging_skipped": True}

    workspace.mkdir(parents=True, exist_ok=True)
    target = workspace / "environment"
    if target.exists():
        # Existing workspace state — preserve.
        return {"environment_dir": str(target), "staging_skipped": False}
    shutil.copytree(env_src_path, target)
    return {"environment_dir": str(target), "staging_skipped": False}
...
def verify_tblite(
    ...
    *,
    ...
    if not test_sh:
        return VerifyResult(
            passed=False, status="missing_test_sh", detail="task payload has no test_sh"
        )
    ...
    if tests_tar:
        try:
            ...
            return VerifyResult(
                passed=False,
                status="tests_tar_extract_error",
    ...
    try:
        ...
        if run_proc.returncode != 0:
            return VerifyResult(
                passed=False,
                status="container_start_error",
                detail=run_proc.stderr[-500:],
            )

        # Inject /tests + /logs/verifier
        subprocess.run(
            ["docker", "exec", cname, "mkdir", "-p", "/tests", "/logs/verifier"],
            capture_output=True,
            text=True,
            timeout=30,
        )
        ...
        subprocess.run(
            ["docker", "rm", "-f", cname], capture_output=True, text=True, timeout=60
        )
...
def verify_swebench(
    ...
    *,
    ...
    payload = task.get("success_check_payload") or {}
    instance_id = (
        task.get("extra", {}).get("instance_id") or task["task_id"].split("/", 1)[-1]
    )
    ...
    if not repo_dir.exists():
        return VerifyResult(
            passed=False,
            status="repo_missing",
            detail=f"expected staged repo at {repo_dir}; did stage_workspace run?",
        )
    ...
    return VerifyResult(
        ...
        status="ok" if passed else "test_failed",
        detail=(
            f"resolved={resolved} report={report_glob[0].name} rc={proc.returncode}"
        ),
        artifacts={"patch": str(patch_path), "report": str(report_glob[0])},
    )


# ---------------------------------------------------------------------------
# SkillsBench verifier
# ---------------------------------------------------------------------------


def verify_skillsbench(
    task: Dict[str, Any],
    run_dir: Path,
) -> VerifyResult:
    """Delegate to :mod:`skill_evolve.benchmark.skillsbench_verifier`.

    The actual subprocess plumbing (bench CLI vs Dockerfile-build
    fallback) lives in the dedicated module so this dispatcher stays
    a one-liner like the others.
    """
    from skill_evolve.benchmark.load import Task as _Task
    from skill_evolve.benchmark.skillsbench_verifier import verify as _do_verify
    ...
    if isinstance(task, _Task):
        ...
        task_obj = _Task(
            ...
            source=task.get("source", "skillsbench"),
            prompt=task.get("prompt", ""),
            success_check_kind=task.get("success_check_kind", "skillsbench_test_sh"),
            success_check_payload=task.get("success_check_payload") or {},
            timeout_s=int(task.get("timeout_s", 600)),
            stage=int(task.get("stage", 1)),
            skill_relevance=task.get("skill_relevance", ""),
            extra=task.get("extra", {}) or {},
        )
    return _do_verify(task_obj, run_dir)
...
_VERIFIERS = {
    ...
    "skillsbench_test_sh": verify_skillsbench,
}
