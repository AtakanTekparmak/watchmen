...
> For `/implement DAYCARE_SPEC.md`. All subagents receive the full spec as context.
> Working directory: `watchmen-fukara/daycare/`. Watchmen source at `../watchmen/src/watchmen/`.
> Key imports: `from watchmen.agent import Agent`; `from watchmen import providers as _providers`;
> `from watchmen.providers import Provider`. Use `uv` for all package ops.
> Ruff config: line-length 120, `select = ["F", "E9"]`, `target-version = "py311"`.

- [ ] **1. Package scaffold + corpus layer + anonymizer**
  - [ ] 1.1 Create `daycare/pyproject.toml`: `name="watchmen-daycare"`, `requires-python=">=3.11"`, entry point `daycare = "daycare.cli:main"`, deps: `click>=8.1`, `tiktoken>=0.7`, `httpx>=0.27`, `rich>=13.7`, `fastembed>=0.4`, `dria-watchmen @ ../watchmen` (editable local). Dev extras: `pytest>=8`, `pytest-cov>=5`, `ruff>=0.4`. Ruff config matching watchmen's. Hatchling build backend, `packages = ["src/daycare"]`.
  - [ ] 1.2 Create `src/daycare/__init__.py` (empty), `src/daycare/py.typed`. Create `tests/__init__.py`, `tests/fixtures/` dir.
  - [ ] 1.3 Implement `src/daycare/providers.py`: re-export `Agent` and `chat_call` from `watchmen.agent`; re-export `Provider` from `watchmen.providers`; add `raw_log_call(response_json, call_id)` — writes to `$OPENROUTER_RAW_LOG_DIR/<timestamp>_<call_id>.json` when env var is set (K5). Export `ping_model(model_slug, provider) -> ProbeResult` wrapping `Provider.probe()`.
  - [ ] 1.4 Implement `src/daycare/corpus.py`: dataclass `Turn(user_text, assistant_text, tool_calls: list[dict], skill_name: str|None, timestamp: str, accepted: bool|None)`; `parse_transcript(path: Path) -> list[Turn]` — reads JSONL line-by-line, reconstructs turns by walking `parentUuid` chain, extracts full content with no truncation (NOT watchmen's `read_session_full`), handles both Claude Code and pi-agent (`claude-agent-acp`) schemas, guards `Path.exists()` with skip-and-log; `match_session_to_project(session_row, projects_json) -> bool` (W6: exact, startswith, basename match); `query_sessions(db_path, project, days=60) -> list[Row]` — SELECT from sessions joined to projects filter; `check_skill_name_column(db_path) -> bool` — returns False if column missing or all-NULL, logs to doctor.json.
  - [ ] 1.5 Implement `src/daycare/anonymize.py`: `strip(text: str, context: AnonymizeContext) -> str` — apply regex replacements in order: UUIDs → `<SESSION_ID>`, absolute paths under `/Users/<name>/` → `/<USER>/`, email addresses → `<EMAIL>`, OR API keys (`sk-or-...`) → `<OR_KEY>`, GitHub repo slugs from `context.project_slugs` → `<REPO_SLUG>`, skill slugs from `context.skill_slugs` → `<SKILL_SLUG>`, `project_dir` matches → `<PROJECT_ROOT>`. `AnonymizeContext` dataclass holds the project-specific sets. Add `tool_call_strip(tool_call: dict, ctx) -> dict` for stripping tool input fields (MCP paths).
  - [ ] 1.6 Implement `src/daycare/watchdog.py`: `Watchdog` class — `start(budget_seconds: int)` installs `signal.SIGALRM` handler that sets `_stop_flag`; `should_stop() -> bool`; `cancel()` removes handler. On platforms without SIGALRM (Windows), fall back to a threading.Timer. Budget fires at `budget_seconds - 600` (10-min grace per K9).

- [ ] **2. Eval pipeline** *(depends on stream 1)*
  - [ ] 2.1 Implement `src/daycare/runner.py`: `RunResult(score: float, completion: str, completion_len_tokens: int, error: str|None)`; `run_rollout_subprocess(prompt, skill_system_prompt, model, provider, seed, temperature, max_tokens) -> RunResult` — spawns a fresh Python subprocess (K10 subprocess isolation) that makes one OR completion call and returns JSON; parent deserialises. Includes a `build_skill_system_prompt(bundle_dir: Path) -> str` that strips SKILL.md frontmatter and inlines scripts per the injection format spec.
  - [ ] 2.2 Implement `src/daycare/verifier.py`: `score_single(prompt, rubric, completion, judge_model, provider) -> float` — single judge call with the 1d scoring prompt template (from spec §"1d: calibration scoring prompt"); JSON parse with one retry; on parse failure log `judge_parse_error` and return `None`; `aggregate_rollouts(results: list[RunResult|None]) -> float` — W9: mean of non-None scores, full-failure → 0.0; `smoke_3_check(eval_rows, bundle_dir, model, provider, seed) -> bool` — 3 random holdout evals × 1 rollout each, returns False if all 3 score 0.0 (K6); `score_bundle(eval_rows, split, bundle_dir, model, judge_model, provider, rollouts, max_workers) -> EvalSummary` — parallel scoring within `max_workers` workers, returns `EvalSummary` dataclass with `holdout_score, by_type, by_accepted, by_length_quartile, invocations_render, unused_render, n_holdout, successful_evals`; assert `successful_evals / n_holdout >= 0.90` else raise `PartialScoringError`.
  - [ ] 2.3 Implement `src/daycare/eval_builder.py` Phase 1a–1d: `pull_sessions(db_path, project, days, projects_json) -> list[Turn]` using `corpus.query_sessions` + `corpus.parse_transcript`; hard discard at 1b for live-infra triggers (`ssh root@`, `nvidia-smi`, `tmux list-sessions`, `tail -f /`) and safety refusals; judge-call classify using 1b prompt template; acceptance signal with regex-first then LLM fallback; theme-boost using analyst `_running.md`; rubric generation using 1c prompt template; calibration scoring with `n_rollouts=3` (not 1 — failure mode #1 defense); discard score==0 and score>=0.9; log all discards.
  - [ ] 2.4 Implement `src/daycare/eval_builder.py` Phase 1e–1f: semantic dedup using `fastembed.TextEmbedding("BAAI/bge-small-en-v1.5")` — embed `anonymized_prompt`, cosine-similarity cluster at threshold 0.92, cap 5 per cluster, assert ≥30 distinct clusters; stratified 50/50 split by type+accepted, assert `n_holdout_per_type ≥ 3` per present type; anonymize all fields via `anonymize.strip()`; add `baseline_completion_len_tokens` field; round-trip sanity gate 1f (score reference ≥ 0.90 + structural assertions for `script_gen`/`skill_invoke`); write `eval_set.jsonl`.

- [ ] **3. Mutation engine** *(depends on stream 1, parallel with stream 2)*
  - [ ] 3.1 Implement `src/daycare/selector.py`: `rank_skills(db_path, bundle_dir, project, days=60, running_md_path=None) -> list[SkillRank]` — for each slug: `traffic_score` from `tool_calls` (with fallback to JSONL scan per W1); `error_boost`; `analyst_boost` 1.5× if slug in `_running.md` "Skill candidates" section; `priority = traffic_score × (1 + error_boost) × analyst_boost`; cold-start fallback: if all traffic_scores are 0, rank by `_manifest.json` mtime descending. Write ranking to `selector_log.md`.
  - [ ] 3.2 Implement `src/daycare/mutator.py`: `parse_sentinel_blocks(text: str) -> list[FileOp]` where `FileOp` is `ADD_FILE | EDIT_FILE | DELETE_FILE | REWRITE_FOLDER`; validation: reject `..` or `/`-rooted paths, reject ADD for existing paths, reject EDIT/DELETE for missing paths, reject unterminated blocks; `apply_ops(bundle_dir, ops) -> Path` — writes to a temp candidate dir; `shebang_insurance(path)` — prepend `#!/usr/bin/env python3` or `#!/bin/bash` if missing (K7); `validate_scripts(candidate_dir) -> list[str]` — run `py_compile` and `bash -n` on each script, return error list; `hash_bundle(bundle_dir) -> str` — SHA256 of sorted file contents concatenation.
  - [ ] 3.3 Implement `src/daycare/leak_scanner.py`: `build_fingerprint_set(eval_set_path) -> set[str]` — all `source_session` IDs, all absolute paths appearing in any `prompt` field, all 12-char+ n-grams appearing in ≥2 different `prompt` fields (cross-eval recurrence, R5 defense); add project slugs and OR keys from `projects.json`; `scan(candidate_dir, fingerprints) -> list[Leak]` — scan all file contents; `enforce_policy(leaks, policy: Literal["zero","warn"]) -> bool` — returns True if candidate should be rejected.

- [ ] **4. Anchor + evolution loop** *(depends on streams 2 and 3)*
  - [ ] 4.1 Implement `src/daycare/anchor.py`: `run_anchor(run_dir, bundle_dir, eval_set, model, judge_model, provider, rollouts, seed, temperature) -> EvalSummary` — snapshot bundle to `iter_0/bundle/`, call `verifier.score_bundle()` on holdout, write `iter_0/eval_summary.json` and `iter_0/sampling.json` (includes `model_version_pin` and `injection_format_hash`). If bundle is empty, set `run.json.iter_0_is_baseline_a = true`.
  - [ ] 4.2 Implement `src/daycare/evolve.py` Phase 3a — `build_weakness_report(run_dir, iter_n, train_evals, best_bundle, model, judge_model, provider, rollouts, baseline_len_quartiles) -> Path`: score train slice, pull bottom-quartile rows, cluster failure modes, build length-quartile table (M3), build invocations/unused telemetry by parsing weak-model completions for skill XML blocks (K2); write `weakness_report.md`; proposer reads only `anonymized_*` fields.
  - [ ] 4.3 Implement `src/daycare/evolve.py` Phase 3b — `propose_candidates(run_dir, iter_n, K, weakness_report, best_bundle, eval_set, proposer_model, provider, run_json) -> list[Path]`: build tool list (10 tools per spec §"Proposer agent tool inventory"); assign clusters round-robin with temperature schedule `[0.3,0.6,0.9,0.3,0.6,0.9]`; run K `watchmen.Agent` invocations with `max_iter=8` and `max_cost_usd` ceiling; parse sentinel blocks via `mutator`; run shebang insurance; run `py_compile`/`bash -n` validation; log `parse_error`/`validate_error` to `history.jsonl`.
  - [ ] 4.4 Implement `src/daycare/evolve.py` Phase 3b.5–3d — `score_and_select(candidates, run_dir, iter_n, best_bundle, holdout_evals, fingerprints, model, judge_model, provider, rollouts, lambda_n, parent_tokens, epsilon, max_workers) -> tuple[Path|None, EvalSummary|None]`: leak-scan each candidate (reject on zero-policy hit); smoke-3 short-circuit (K6); full holdout scoring with `verifier.score_bundle()`; compute fitness with calibrated λ (M5); select winner; detect stall (3 consecutive all-failed iters → set `STOP_AFTER_THIS_ITER`); append `history.jsonl`; write `mutation_log.md`.
  - [ ] 4.5 Implement `src/daycare/evolve.py` — `run_evolution(run_dir, ...) -> None` outer loop: check watchdog `should_stop()` at each iter boundary; call 3a→3b→3b.5→3d in sequence; append `metrics.json`; context reset (3e) — all load-bearing state is on disk, no in-memory carry-over between iters.

- [ ] **5. Controls + CLI + daemon** *(depends on stream 4)*
  - [ ] 5.1 Implement `src/daycare/controls.py`: `run_baseline_a(run_dir, holdout_evals, model, judge_model, provider, rollouts, epsilon) -> BaselineResult` — empty bundle, 5 rollouts, hard-block check at `2ε` (J1), write `run.json.promote_blocked = True` if fails; `run_baseline_b(run_dir, train_evals, holdout_evals, ...) -> BaselineResult` — construct 5-example few-shot SKILL.md (anonymized), score, write `baseline_b_fewshot/`; `run_baseline_c(run_dir, holdout_evals, teacher_model, ...) -> BaselineResult` — score with teacher model + empty skill, compute `gap_closed = (best - a) / (c - a)`, write `baseline_c_teacher/`; `run_reproducibility(run_dir, ...) -> dict` — optional re-run with seed+1 (§4c).
  - [ ] 5.2 Implement `src/daycare/finalize.py`: `write_optimized(run_dir, best_bundle_path, slug)` — copy to `optimized/<slug>/`; `promote(run_dir, slug, project, watchmen_home) -> None` — copy `optimized/<slug>/` → `bundles/<project>/skills/<slug>/`, append to `_pinned.json` flat array (W4), append provenance block to `_curation_log.md`, update `_manifest.json` mtimes; `write_summary(run_dir, metrics, baseline_results)` — append `## Final summary` to `scratchpad.md`, print rich summary table.
  - [ ] 5.3 Implement `src/daycare/cli.py`: Click group `daycare`; `doctor` command — check corpus.db, `skill_name` column (W1), OR pings, OPENROUTER_RAW_LOG_DIR, write `doctor.json`; `eval-build <project>` command — Phase 0 bootstrap + Phase 1 only + print cluster report; `run <project>` command — full Phases 0–5, all flags from CLI surface table, J2 multi-project survey on first run (run eval-build on all projects if no prior run exists, pick richest); `promote <project> <slug>` command; `runs` command — read all `run.json` files, print table with Δ scores and gap_closed; `daemon {install,uninstall,run}` subgroup.
  - [ ] 5.4 Implement `src/daycare/daemon.py`: `install_launchd(watchmen_home)` — write `~/Library/LaunchAgents/com.daycare.daemon.plist` with 2h StartInterval; `install_systemd(watchmen_home)` — write `~/.config/systemd/user/daycare.service`; `run_daemon(watchmen_home)` — 2h cycle: re-scan corpus for new sessions, refresh selector ranking, check if daily run due (03:00 local), run `daycare run` on highest-priority skill; weekly: re-run Phase 1d calibration; integrate SIGALRM watchdog per run.

- [ ] **6. Tests + integration** *(depends on all streams)*
  - [ ] 6.1 Unit tests `tests/test_corpus.py`: test `parse_transcript()` on a synthetic 10-turn JSONL fixture (create fixture in `tests/fixtures/sample_transcript.jsonl`); test `match_session_to_project()` for exact/startswith/basename cases; test `check_skill_name_column()` with both schema variants.
  - [ ] 6.2 Unit tests `tests/test_mutator.py`: test all 4 sentinel block types round-trip; test traversal guard rejects `../escape`; test unterminated block raises; test shebang insurance adds missing shebangs; test `hash_bundle()` is deterministic.
